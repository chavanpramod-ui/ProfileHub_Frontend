...
const AchievementPost = ({ post, isOwner, onEdit, onDelete, compact = false }) => {
  ...
  return (
    <div className={compact ? 'group p-0' : 'group bg-white border border-slate-200 rounded-4xl p-6 transition-all hover:shadow-[0_24px_60px_-24px_rgba(15,23,42,0.18)]'}>
      <div className={compact ? 'flex flex-col gap-3' : 'flex justify-between items-start'}>
        <div className={compact ? 'flex items-start gap-3 w-full' : 'flex gap-4'}>
          ...
          <div className="flex-1">
            <h3 className="text-base font-bold text-slate-900 group-hover:text-[#0f4c81] transition-colors">
              {post.title}
            </h3>
            
            <div className="flex flex-wrap items-center gap-2 mt-1 text-[13px] text-slate-500 font-medium">
              <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-600">{post.provider || "Professional Update"}</span>
            ...
            <p className="mt-3 text-slate-600 text-sm leading-6 whitespace-pre-wrap">
        ...
        {isOwner && (
          <div className="flex gap-1">
            <button onClick={() => onEdit(post)} className="p-2 text-gray-400 hover:text-[#0f4c81] hover:bg-[#eff6ff] rounded-lg transition-all">
              <Edit2 size={16} />
            </button>
            <button onClick={() => onDelete(post._id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
              <Trash2 size={16} />
            </button>
          </div>
        )}
