  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-[120] p-4">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] p-8 relative shadow-2xl animate-in zoom-in duration-200">
        <button onClick={onClose} className="absolute top-6 right-6 text-slate-muted hover:text-white"><X size={24} /></button>
        ...
        <div className="max-h-80 overflow-y-auto pr-2 custom-scrollbar">
          {followers?.length > 0 ? (
            followers.map((follower) => (
              ...
              >
                <div className="w-10 h-10 bg-deep-blue rounded-full flex items-center justify-center font-bold">
                  {follower.username[0].toUpperCase()}
                </div>
                <div>
                  <p className="text-white font-bold text-sm">{follower.username}</p>
                  <p className="text-slate-muted text-xs">{follower.bio || "MERN Developer"}</p>
                </div>
              </div>
            ))
          ) : (
            <p className="text-center text-slate-muted py-10">No followers yet. Share your Hub!</p>
          )}
        </div>
      </div>
    </div>
